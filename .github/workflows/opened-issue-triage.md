...
1. Fetch the list of labels available in this repository. Use 'gh label list' command to 
   fetch the labels. 

2. Retrieve the issue content using the `get_issue` tool. If the issue is obviously spam, 
   or generated by bot, or something else that is not an actual issue to be worked on, then add an issue 
   comment to the issue with a one-sentence analysis, add the `spam` label, and exit the workflow.

3. Use the GitHub tools to gather additional context about the issue:
...
5. Write notes, ideas, nudges, resource links, debugging strategies and/or reproduction steps for the 
   ...
   ask for clarying information in the issue comments and tag the user who opened the issue to get their attention.

6. Select appropriate labels from the available labels list provided above:

   - Choose labels that accurately reflect the issue's nature
   - Be specific but comprehensive
   - Select priority labels if you can determine urgency (high-priority, medium-priority, or low-priority)
   - Search for similar issues, and if you find similar issues consider using the 'duplicate' label if appropriate. 
...
8. Add an issue comment to the issue with your analysis:
   ...
   - Suggest resources or links that might be helpful for resolving the issue or 
     learning skills related to the issue or the particular area of the codebase affected by it
