...
- [ai-gateway.yaml](ai-gateway.yaml) defines a root AI Gateway resource with
  a nested OpenAI provider, env vault, data plane node, policy, consumer, agent,
  consumer group, model that targets that provider, and a conversion-only MCP
  Server.
- [federated](federated) shows a multi-folder
  ...
  Servers, vaults, and nodes that reference the shared gateway.
