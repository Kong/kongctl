...
func (e *tailEventEmitter) EmitRecords(records [][]byte) error {
	...
	for _, record := range records {
		...
		// #nosec G705 -- audit logs are intentionally streamed as raw CLI output.
		if _, err := fmt.Fprintln(
			e.out,
			strings.TrimRight(string(outputRecord), "\n"),
		); err != nil {
			return err
		}
	}
...
func (e *tailEventEmitter) warn(format string, args ...any) {
	...
	_, _ = fmt.Fprintf(
		...
		format+"\n",
		args...,
	) // #nosec G705 -- terminal warning text is intentionally unescaped.
}
