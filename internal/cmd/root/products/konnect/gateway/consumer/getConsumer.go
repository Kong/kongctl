...
func findConsumerByUsername(helper cmd.Helper, cfg config.Hook, sdk *kk.SDK, cpID, username string) (*kkComps.Consumer, error) {
...
func renderConsumers(helper cmd.Helper, outType cmdCommon.OutputFormat, printer cli.PrintFlusher, consumers []kkComps.Consumer) error {
	switch outType {
	case cmdCommon.TEXT:
		records := make([]consumerDisplayRecord, 0, len(consumers))
		for i := range consumers {
			records = append(records, consumerToDisplayRecord(&consumers[i]))
		}
		if printer != nil {
	...
	default:
		if printer != nil {
			if len(consumers) == 1 {
				printer.Print(consumers[0])
			} else {
				printer.Print(consumers)
			}
		}
		return nil
	}
