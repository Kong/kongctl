...
var (
	...
	%[1]s get konnect gateway service --control-plane <control-plane-id>
	`, meta.CLIName)))
)
...
func (c *getServiceCmd) runListByName(ctx context.Context, cpID string, name string,
	...
	allData, err := helpers.GetAllGatewayServices(ctx, requestPageSize, cpID, kkClient)
	if err != nil {
...
func (c *getServiceCmd) runGet(ctx context.Context, cpID string, id string,
	kkClient *kk.SDK, helper cmd.Helper, printer cli.Printer,
) error {
	res, err := kkClient.Services.GetService(ctx, cpID, id)
	if err != nil {
		attrs := cmd.TryConvertErrorToAttrs(err)
...
func (c *getServiceCmd) runList(ctx context.Context, cpID string,
	kkClient *kk.SDK, helper cmd.Helper, cfg config.Hook, printer cli.Printer,
) error {
	requestPageSize := int64(cfg.GetInt(kkCommon.RequestPageSizeConfigPath))
...
func (c *getServiceCmd) runE(cobraCmd *cobra.Command, args []string) error {
	...
	if cpID == "" {
		...
		var err error
		cpID, err = helpers.GetControlPlaneID(ctx, kkClient, cpName)
		if err != nil {
			attrs := cmd.TryConvertErrorToAttrs(err)
	...
	if len(helper.GetArgs()) == 1 { // validate above checks that args is 0 or 1
		...
		if !isUUID {
			...
			return c.runListByName(ctx, cpID, id, kkClient, helper, cfg, printer)
		}

		return c.runGet(ctx, cpID, id, kkClient, helper, printer)
	}

	return c.runList(ctx, cpID, kkClient, helper, cfg, printer)
}
