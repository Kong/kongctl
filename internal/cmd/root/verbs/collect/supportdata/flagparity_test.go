...
func TestAllConfigFieldsMapped(t *testing.T) {
	...
	mappedFields := map[string]string{
		...
		"TLSSkipVerify":        "--tls-skip-verify flag / config support_data.tls_skip_verify",
		"CACertPath":           "--ca-cert flag / config support_data.ca_cert_path",
	...
	configType := reflect.TypeFor[collector.Config]()
