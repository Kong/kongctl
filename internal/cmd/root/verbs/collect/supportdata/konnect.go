...
func runKonnect(
	...
	if len(collectorCfg.RBACHeaders) == 0 || collectorCfg.RBACHeaders[0] == "" {
		return fmt.Errorf(
			"a Konnect Personal Access Token is required; use --pat or set %s in config",
			konnectcommon.PATConfigPath,
		)
	}

	// Resolve the output format before collecting so an invalid format fails
	// immediately rather than after a long collection run.
	...
	return FormatOutput(helper.GetStreams(), outputFormat, result)
}
...
func buildKonnectConfig(
	...
	// Resolve the Konnect address, with flags taking precedence over config.
	// Config is only consulted when neither flag is set, so an invalid region
	// in the config file cannot block an otherwise valid --base-url/--region.
	switch {
	case flags.BaseURL != "":
