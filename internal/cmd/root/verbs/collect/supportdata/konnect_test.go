...
func TestBuildKonnectConfig_BaseURLAndRegion(t *testing.T) {
	...
	// An invalid region in the config file must not block a valid flag. The
	// config is only consulted when neither --base-url nor --region is set.
	t.Run("base-url flag overrides invalid region in config", func(t *testing.T) {
		mock := newMockConfig()
		mock.GetStringMock = func(path string) string {
			if path == konnectcommon.RegionConfigPath {
		...
		cfg, err := buildKonnectConfig(mock, commonFlags, flags)
		require.NoError(t, err,
			"an invalid region in config should not block a valid --base-url")
		assert.Equal(t, "https://custom.api.konghq.com", cfg.KongAddr)
	})

	t.Run("region flag overrides invalid region in config", func(t *testing.T) {
		mock := newMockConfig()
		mock.GetStringMock = func(path string) string {
			if path == konnectcommon.RegionConfigPath {
				return "invalid region!"
		...
		cfg, err := buildKonnectConfig(mock, commonFlags, flags)
		require.NoError(t, err,
			"an invalid region in config should not block a valid --region")
		assert.Equal(t, "https://eu.api.konghq.com", cfg.KongAddr)
	})

	t.Run("invalid region in config still errors when no flag is set", func(t *testing.T) {
		mock := newMockConfig()
		mock.GetStringMock = func(path string) string {
			if path == konnectcommon.RegionConfigPath {
				return "invalid region!"
		...
		_, err := buildKonnectConfig(mock, &CommonFlags{}, &konnectFlags{})
		assert.Error(t, err,
			"an invalid region in config should error when there is no flag to fall back on")
	})
}
...
func TestBuildKonnectConfig_RuntimeFlags(t *testing.T) {
	t.Run("runtime flag sets Runtime", func(t *testing.T) {
		mock := newMockConfig()
		commonFlags := &CommonFlags{}
		flags := &konnectFlags{Runtime: runtimeDocker}

		cfg, err := buildKonnectConfig(mock, commonFlags, flags)
		require.NoError(t, err)

		assert.Equal(t, runtimeDocker, cfg.Runtime)
...
func TestKonnectKubernetesRequiresNamespace(t *testing.T) {
	t.Run("kubernetes without namespace returns error", func(t *testing.T) {
		...
		// Assert against the validation runKonnect actually performs, rather
