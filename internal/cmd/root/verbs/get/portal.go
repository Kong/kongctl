...
// NewDirectPortalCmd creates a portal command that can be used directly under the get verb
// This enables the Konnect-first pattern: kongctl get portals
func NewDirectPortalCmd() (*cobra.Command, error) {
	// Create the portal command with the same logic as the konnect portal command
	baseCmd, err := portal.NewPortalCmd(verbs.Get, addKonnectFlags, konnectPreRunE)
	if err != nil {
		return nil, err
	}
