...
func newThemesCmd() (*cobra.Command, error) {
	cmd := &cobra.Command{
		Use:   "themes",
...
func runListThemes(helper cmd.Helper) error {
	...
	if outFormat != cmdcommon.INTERACTIVE {
	...
	useColor := shouldRenderColor(cfg, outFormat, streams.Out)
...
func shouldRenderColor(cfg config.Hook, out cmdcommon.OutputFormat, outWriter io.Writer) bool {
	if out != cmdcommon.INTERACTIVE {
		return false
	}
