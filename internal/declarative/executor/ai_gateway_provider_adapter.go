...
// AIGatewayProviderAdapter implements ResourceOperations for AI Gateway Providers.
type AIGatewayProviderAdapter struct {
	client *state.Client
}

// NewAIGatewayProviderAdapter creates a new AI Gateway Provider adapter.
...
func (a *AIGatewayProviderAdapter) MapCreateFields(
	...
	create *kkComps.CreateAIGatewayProviderRequest,
) error {
	...
	if err != nil {
		return fmt.Errorf("failed to encode AI Gateway Provider create payload: %w", err)
	}
	if err := json.Unmarshal(data, create); err != nil {
		return fmt.Errorf("failed to decode AI Gateway Provider create payload: %w", err)
	}
	return nil
}
...
func (a *AIGatewayProviderAdapter) MapUpdateFields(
	...
	_ *ExecutionContext,
	fields map[string]any,
	update *kkComps.UpdateAIGatewayProviderRequest,
	_ map[string]string,
) error {
	payload, err := aiGatewayProviderPayloadFromFields(fields)
	if err != nil {
	...
	data, err := json.Marshal(payload)
	if err != nil {
		return fmt.Errorf("failed to encode AI Gateway Provider update payload: %w", err)
	}
	if err := json.Unmarshal(data, update); err != nil {
		return fmt.Errorf("failed to decode AI Gateway Provider update payload: %w", err)
	}
	return nil
}
...
func (a *AIGatewayProviderAdapter) getAIGatewayIDFromExecutionContext(execCtx *ExecutionContext) (string, error) {
	...
	return "", fmt.Errorf("AI Gateway ID required for AI Gateway Provider operations")
}
