...
func injectSecretWriteValidationPlaceholders(change *planner.PlannedChange) error {
	if change == nil {
		return nil
	}
	for _, intent := range change.SecretWrites {
		if err := setSecretField(change.Fields, decodeSecretPointer(intent.Field), "payload-validation-secret"); err != nil {
			return fmt.Errorf("failed to validate secret field %s: %w", intent.Field, err)
		}
	}
	return nil
