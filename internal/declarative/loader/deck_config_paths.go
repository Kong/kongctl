...
func (l *Loader) resolveDeckConfigPaths(rs *resources.ResourceSet, baseDir string, rootDir string) error {
	if rs == nil {
		return nil
	}
	...
	for i := range rs.ControlPlanes {
		...
		cp.SetDeckBaseDir(baseDirAbs)
		if err := validateDeckFiles(cp.Deck.Files, baseDirAbs, rootDirAbs); err != nil {
			return fmt.Errorf("control_plane %q _deck files: %w", cp.Ref, err)
		}
	}
