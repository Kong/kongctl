...
func TestLoader_TagProcessing(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(portalFile, []byte(portalNameContent), 0600))
...
info:
  ...
  contact:
	...
	require.NoError(t, os.WriteFile(specFile, []byte(openapiContent), 0600))
	...
	require.NoError(t, os.WriteFile(descFile, []byte(descContent), 0600))
...
apis:
  - ref: test-api
	...
	require.NoError(t, os.WriteFile(mainFile, []byte(mainContent), 0600))
  ...
  - ref: test
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, os.WriteFile(testFile, []byte(tt.yamlContent), 0600))
