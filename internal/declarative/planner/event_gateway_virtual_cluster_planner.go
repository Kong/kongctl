...
func (p *Planner) planEventGatewayVirtualClusterChanges(
	...
	p.logger.Debug(
		"Planning Event Gateway Virtual Cluster changes",
...
func (p *Planner) planVirtualClusterChangesForExistingGateway(
	...
	p.logger.Debug(
		"Planning changes for existing gateway virtual clusters",
		"gateway_id", gatewayID,
		"gateway_ref", gatewayRef,
		"desired_count", len(desired),
	)
	...
	p.logger.Debug(
		"Fetched current virtual clusters",
		"gateway_id", gatewayID,
	...
	for _, desiredCluster := range desired {
		...
		if !exists {
			// CREATE
			p.logger.Debug(
				"Planning virtual cluster CREATE",
			...
			// CHECK UPDATE
			p.logger.Debug(
				"Checking if virtual cluster needs update",
				"cluster_name", desiredCluster.Name,
			...
			if needsUpdate {
				p.logger.Debug(
					"Planning virtual cluster UPDATE",
					"cluster_name", desiredCluster.Name,
					"cluster_id", current.ID,
	...
	if plan.Metadata.Mode == PlanModeSync {
		for name, current := range currentByName {
			if !desiredNames[name] {
				p.logger.Debug(
					"Planning virtual cluster DELETE (sync mode)",
					"cluster_name", name,
					"cluster_id", current.ID,
				)
				p.planVirtualClusterDelete(
					namespace, gatewayRef, gatewayName, gatewayID, current.ID, name,
					current.Destination.ID, current.Destination.Name, plan,
				)
			}
		}
	}
...
func (p *Planner) planVirtualClusterCreatesForNewGateway(
	...
	p.logger.Debug(
		"Planning virtual cluster creates for new gateway",
		"gateway_ref", gatewayRef,
		"gateway_change_id", gatewayChangeID,
...
func (p *Planner) planVirtualClusterDelete(
	...
	} else if backendClusterName != "" {
		change.References = map[string]ReferenceInfo{
			FieldEventGatewayBackendClusterID: {LookupFields: map[string]string{FieldName: backendClusterName}},
		}
	}

	p.logger.Debug(
		"Enqueuing virtual cluster DELETE",
