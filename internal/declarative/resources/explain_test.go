...
func TestRenderScaffoldYAML_AIGatewayAuthStrategyConsumerClaims(t *testing.T) {
	subject, err := ResolveExplainSubject("ai_gateway_auth_strategy")
	require.NoError(t, err)
...
func TestSecretFieldsPreferExplicitSecretScaffolds(t *testing.T) {
	tests := []struct {
		path     string
		expected string
	}{
		{
			path:     "ai_gateway_model_provider",
			expected: `value: !secret {parts: ["Bearer ", !env MODEL_PROVIDER_API_KEY]}`,
		},
		{
			path:     "ai_gateway_consumer_credential",
			expected: "api_key: !secret {source: !env AI_GATEWAY_CONSUMER_API_KEY}",
		},
		{
			path:     "portal_identity_provider",
			expected: "client_secret: !secret {source: !env PORTAL_OIDC_CLIENT_SECRET}",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.path, func(t *testing.T) {
			...
			scaffold, err := RenderScaffoldYAML(subject)
			require.NoError(t, err)
			assert.Contains(t, scaffold, tt.expected)
		})
	}
}
