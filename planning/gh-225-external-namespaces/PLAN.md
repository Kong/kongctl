...
3. **Testing**
   - Author a new `test/e2e/scenarios/external/...` scenario that syncs a config containing only an external portal plus a team-owned 
        API/Publication and asserts that NO portal DELETE is planned (desired behavior).
   - Add supplemental unit tests for loader validation and namespace filtering.
4. **Documentation**
   - Expand `docs/declarative.md` namespace section to cover:
     ...
     * Usage and behavior of the existing `--require-any-namespace` / `--require-namespace` flags.
